    ...
    <>
      {Object.keys(matchPointsTable).length > 0 ? (
        Object.entries(matchPointsTable).map(([groupName, teams]) => (
          <div className="md:my-6 pt-8 mx-4">
            <div
              key={groupName}
              className="gap-1 d-flex flex-col lg:w-[60%] md:w-[90%] mx-auto pb-10"
            >
              <Typography variant="h2" gutterBottom>
                {groupName} {/* Dynamic Group Name */}
              </Typography>
              <div className="flex flex-wrap justify-between xl:gap-36 bg-gradient-to-r from-[#39441d] to-[#141815] py-3 rounded-lg lg:px-[10%] px-[5%]">
                <span>Team</span>
        ...
        <div className="text-center text-white my-10 py-4 bg-[#242424] w-50 mx-auto rounded-lg">
          No Data Found
        </div>
      )}
    </>
